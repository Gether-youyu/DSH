...
一套让 DSH(DeepSeek Harness)通过**飞书机器人**和**邮件**接入手机的消息网关系统。手机发消息 → DSH 执行 → 回复回手机;电脑端同一会话实时同步。附一键安装、崩溃自愈、运行监控。
...
| 邮件通道 | 邮件收发(备用通道) |
| 每日推送 | 每天 19:59 邮件总结当天工作(系统索引+任务摘要,不扫全文) |
